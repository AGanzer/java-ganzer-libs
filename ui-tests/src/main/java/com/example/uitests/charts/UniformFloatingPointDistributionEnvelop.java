...
import de.ganzer.core.random.UniformFloatingPointDistribution;
import de.ganzer.core.validation.NumberValidator;
import javafx.fxml.FXMLLoader;

public class UniformFloatingPointDistributionEnvelop extends DistributionEnvelop<Double> {
    @Override
    ...
    protected void initController(DistributionSettingsController controller) {
        ...
        settingsController.setValue1LabelText("M_inimum Value:");
        settingsController.setValue2LabelText("M_aximum Value:");
    ...
    protected Distribution<Double> createDistribution(DistributionSettingsController controller) {
        var settingsController = (TwoValueSettingsController)controller;
        var min = settingsController.getValue1();
        var max = settingsController.getValue2();

        if (min <= max)
            return new UniformFloatingPointDistribution(min, max);

        return new UniformFloatingPointDistribution(max, min);
    }
}
